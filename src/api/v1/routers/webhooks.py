...
async def webhook_create(
    data: WebhookCreate, service: WebhookService = Depends(get_webhook_service)
):
    """Создает подписку"""
    try:
...
async def webhook_get(
    ...
    try:
        ...
        response_subscriptions = [
            WebhookResponse.model_validate(subscription)
            for subscription in subscriptions
        ]
        return PaginatedWebhookResponse.create(
            items=response_subscriptions, total=total, offset=offset, limit=limit
        )
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def webhook_delete(
    webhook_id: int, service: WebhookService = Depends(get_webhook_service)
):
...
async def get_deliveries(
    ...
    try:
        ...
        response_dels = [
            WebhookDeliveryResponse.model_validate(delivery) for delivery in dels
        ]

        return PaginatedDeliveriesResponse.create(
            items=response_dels, total=total, offset=offset, limit=limit
        )
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
