...
class AnalyticsRepository:
    ...
    async def get_shift_stats(self) -> list[str]:
        query = (
            select(
                ...
                func.sum(case((Product.is_aggregated == True, 1), else_=0)).label(
                    "aggregated"
                ),
            )
    ...
    async def get_top_work_centers(self, limit: int = 10):
        query = (
            select(
            ...
            .outerjoin(Batch, Batch.work_center_id == WorkCenter.id)
            .outerjoin(Product, Product.batch_id == Batch.id)
